...
fun NumismaticsPlatformContext.toTransport(): IReferenceResponse = when (entityType) {
    EntityType.COUNTRY ->
        when (command) {
...
private fun Entities.countryToTransport() = this
    .filterIsInstance<CountryInternal>()
    .first()
    .toTransport()
...
private fun Entities.materialToTransport() = this
    .filterIsInstance<MaterialInternal>()
    .first()
    .toTransport()
...
private fun Entities.sectionToTransport() = this
    .filterIsInstance<SectionInternal>()
    .first()
    .toTransport()
...
private fun MaterialInternal.toTransport() = ItemResponse(
    reference = Material(
        id = id.takeIf { it != MaterialId.EMPTY }?.toLong(),
...
private fun CountryInternal.toTransport() = ItemResponse(
    reference = Country(
        id = id.takeIf { it != CountryId.EMPTY }?.toLong(),
        name = name.takeIf { it.isNotBlank() },
        description = description.takeIf { it.isNotBlank() }
    ),
    permissions = permissions.toTransport { it.toTransport() }
...
private fun SectionInternal.toTransport() = ItemResponse(
    reference = Section(
        id = id.takeIf { it != SectionId.EMPTY }?.toLong(),
        name = name.takeIf { it.isNotBlank() },
        description = description.takeIf { it.isNotBlank() },
        parentId = parentId.takeIf { it != SectionId.EMPTY }?.toLong(),
    ),
    permissions = permissions.toTransport { it.toTransport() }
)
